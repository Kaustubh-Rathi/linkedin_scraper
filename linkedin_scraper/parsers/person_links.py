"""Pure, browser-independent link and contact parsing helpers."""
...
def classify_link(href: str, label: str | None = None) -> Contact:
...
def contact_type_from_heading(heading: str) -> str | None:
...
def merge_contacts(primary: list[Contact], extra: list[Contact]) -> list[Contact]:
    ...
    for contact in primary + extra:
        key = f"{contact.type}:{contact.value.strip()}"
